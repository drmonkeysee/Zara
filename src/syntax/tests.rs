...
mod parsing {
    ...
    fn failed_node_into_unexpected_end_of_parse_discards_rest_of_input() {
        ...
        assert_matches!(r, Ok(ParserOutput::Continuation));
    ...
    fn define_can_redefine_itself() {
        ...
        assert_eq!(val.as_datum().to_string(), "10");
        assert!(et.parsers.is_empty());
    }
    ...
    fn redefined_special_form_treated_like_procedure_call() {
        ...
        let env = TestEnv::default();
        env.binding.bind(env.symbols.get("define"), Value::real(10));
...
mod errors {
    ...
    fn syntax_source() {
        ...
        assert_matches!(inner.downcast_ref::<SyntaxError>().unwrap(), SyntaxError(_));
    }
...
mod partition {
    ...
    fn one_span() {
        ...
        assert_matches!(groups[0][..], [TxtSpan { start: 0, end: 5 }]);
    }
    ...
    fn stacked() {
        ...
        assert_matches!(groups[0][..], [TxtSpan { start: 0, end: 5 }]);
        assert_matches!(groups[1][..], [TxtSpan { start: 3, end: 6 }]);
        assert_matches!(groups[2][..], [TxtSpan { start: 4, end: 8 }]);
    }
    ...
    fn contained() {
        ...
        assert_eq!(groups.len(), 2);
        assert_matches!(groups[0][..], [TxtSpan { start: 0, end: 10 }]);
        assert_matches!(
            groups[1][..],
